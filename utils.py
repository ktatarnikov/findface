...
from keras.layers import Activation, Dense, Input, InputLayer, Lambda, Reshape
...
class CelebA:
    def __init__(self,
        ...
        if train:
            ...
            image_filenames = image_filenames[-5000:]
        input_queue = tf.train.slice_input_producer([image_filenames],
                                                    shuffle=False)
...
def create_encoder(input_dims, base_filters=64, layers=4, latent=512):
    ...
    for i in range(layers):
        encoder.add(
            Conv2D(filters=base_filters * 2**i,
                   kernel_size=(5, 5),
                   strides=(2, 2),
                   padding='same',
                   bias=False))
